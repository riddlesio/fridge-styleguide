...
install_githooks() {
  curl -m 2 -s $GIT_HOOK_BASE_URL/git/$1/hooks/pre-commit > /tmp/pre-commit && mv /tmp/pre-commit .git/hooks/pre-commit
  curl -m 2 -s $GIT_HOOK_BASE_URL/git/$1/hooks/commit-msg > /tmp/commit-msg && mv /tmp/commit-msg .git/hooks/commit-msg
